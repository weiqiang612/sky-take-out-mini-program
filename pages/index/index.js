...
import agentChatBtn from "@/components/agent-chat-btn/agent-chat-btn.vue"
import {
	// 苍穹外卖相关的接口
	userLogin,
	getCategoryList,
...
export default {
	...
	methods: {
		...
		...mapMutations([
			...
			"setMerchantDisplayInfo", //设置首页商户展示信息
			"setUserId",
		]),
		// 从vuex信息
		...mapState([
											...
											userLogin(params)
												.then((success) => {
													if (success.code === 1) {
														...
														if (success.data.id || success.data.userId) {
															_this.setUserId(String(success.data.id || success.data.userId))
														}
														// 保存配送费
														_this.setDeliveryFee(
															success.data.deliveryFee == null
																? 6
																: success.data.deliveryFee
														)
														// 保存商铺信息
														_this.setShopInfo({
														...
														_this.setMerchantDisplayInfo({
															shopId:
																success.data.shopId ||
																_this.merchantDisplayInfo().shopId,
															shopName: success.data.shopName || "苍穹外卖",
															shopAddress:
																success.data.shopAddress ||
																_this.merchantDisplayInfo().shopAddress,
															deliveryFee:
																success.data.deliveryFee == null
																	? _this.merchantDisplayInfo().deliveryFee
																	: success.data.deliveryFee,
															distance:
																success.data.distance ||
																_this.merchantDisplayInfo().distance,
															estimatedDeliveryTime:
																success.data.estimatedDeliveryTime ||
																_this.merchantDisplayInfo()
																	.estimatedDeliveryTime,
														})
														_this.init()
													}
